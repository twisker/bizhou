...
- [**常见问题 FAQ**](./faq.html)
- [**版本与兼容性**](./versions.html) —— 各版本差异、兼容性承诺、历史版本下载、为什么不自动更新。

## 一句话能力清单

加密上传/还原（字节一致）· **云端保险库（换机只需重输主密码）** · 并发上传 · 断点续传 · 内容去重 + 在飞锁 · 云端真实目录树（`mkdir/ls/mv/cp/rename`）· 回收站 · `-r` 递归整树 · 自动备份守护 · 多类型预览（文本/PDF/媒体/压缩包列表）· 分享（码 / 7z-AES）· 多账号 · 网盘配额 · shell 补全。

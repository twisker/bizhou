...
**非目标**：worker_threads 并行加密（网络场景零收益，另行评估）；跨机器/跨进程的云端锁（本轮用本地锁，云端锁记为远期）；内容寻址的全局去重索引（本轮去重靠扫描目标目录）。
...
2. 去重扫描：列目标云端目录的 bundles，逐个解 encMeta 比 contentId。
...
**CLI `bz`：**
- 修改 `packages/cli/src/commands.ts` — `cmdPush` / `cmdPushDir` 集成：预哈希 contentId、去重扫描、日志锁/续传、`--force`、`--concurrency`、消息；`cmdPull` / `cmdPullDir`（S2）集成幂等/续传/原子落地。
...
- **S1 · 健壮上传**（本轮）：contentId 底座 → 并发池 → 上传日志 → cmdPush 集成 → 递归 push 集成。5 个 TDD 任务。

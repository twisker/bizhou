...
| H-02 | 用一个测试百度账号完成 M0 **关键验证**：`bz login` 拿 token 后 `bz push` 大文件 → `bz pull` 校验字节一致 → 确认云端不因"内容不可识别的加密大文件"而限制/封禁（全案前提）。**对接代码已实现并通过 mock 测试，只差真实 OAuth 授权 + 联网跑一遍。** | Sprint 0 (M0) | P0 | 待开始 |

...
- **换机只需重输主密码**：保险库（`vault.json`，内含被包裹的主密钥）默认有一份**加密副本**存在你自己的网盘，新机 `bz login` + `bz unlock` 即自动取回。代价你有权知道：云服务商由此持有该密文，可离线无限次爆破，因此 `bz init` 会**拦下强度不足的主密码**（KDF 也已提升到 scrypt N=2¹⁷）。不愿上云可用 `bz init --no-cloud-vault`，但换机/重装/硬盘损坏将使数据永久锁死，须自行备份 `~/.bizhou`。详见[安全模型](https://twisker.github.io/bizhou/zh/security.html)。

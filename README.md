...
- **换机务必带上密钥根** ⚠️：保险库文件（`vault.json`，内含主密码用的盐与被包裹的 MK）只存在本地密钥根（默认 `~/.bizhou`），**从不上传云端**。换机/重装时必须把它一并带过去，再重输主密码；**只有主密码、或只有恢复密钥，都打不开任何东西**——恢复密钥解的是保险库里那份密文，文件没了它也无从下手。**请现在就备份 `~/.bizhou`**，与恢复密钥分开存放。（v1.1.0 计划支持保险库加密上云，届时换机只需重输主密码。）

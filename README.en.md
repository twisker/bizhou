...
- **Moving machines? Bring your key root** ⚠️: the vault file (`vault.json`, holding the password salt and the wrapped MK) lives **only** in your local key root (`~/.bizhou` by default) and is **never uploaded**. On a new machine or after a reinstall you must bring it along and then re-enter your master password. **Neither the master password alone nor the recovery key alone opens anything** — the recovery key unwraps the ciphertext *inside* that file, so without the file it has nothing to work on. **Back up `~/.bizhou` now**, and keep that backup separate from your recovery key. (v1.1.0 plans to store an encrypted vault in the cloud so a new machine will need only the master password.)

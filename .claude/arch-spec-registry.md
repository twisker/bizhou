...
### 5.1 历史版本与更新策略（人工决策留痕，2026-07-25）

**决策一：历史版本全部归档保留，不下架。**

- npm 保留全部已发布版本（`npm i -g @bizhou/cli@<任意版本>`）；GitHub Releases 保留每版 tarball 与 release notes；每版一个 git tag，可自行构建复现。
- Homebrew tap / Scoop bucket 只带最新版 manifest（两个渠道的常规做法）；需要旧版走 npm 或 Release。
- **理由**：① npm 本来就无法真正下架（超过 72 小时只能 `deprecate`），"只留最新版"在最主要的渠道上不可实现；② 开源加密工具的信任基础之一是**任何人都能取到某个具体版本的源码与产物、独立验证它们对得上**，砍掉历史等于砍掉可复现构建与审计；③ 存储成本为零。
- **代价（接受）**：旧版本的已知问题会长期可安装。对策是把问题与升级路径写在 `site/{zh,en}/versions.md`，而不是靠删包。

**决策二：不做版本发现 / 强制更新，不发任何遥测。**

- `bz` 运行期间**不向除百度网盘 API 之外的任何端点发请求**。升级完全由包管理器驱动（`brew outdated` / `scoop status` / `npm outdated -g` 已覆盖此需求）。
- **理由**：① 版本检查是周期性对外信标，会暴露"这台机器在用这个工具"，与低调定位（PRD §6）和"无遥测"承诺直接冲突；② 自动升级一个**握着用户唯一解密钥匙**的工具，风险高度不对称——升级失败的代价是数据打不开，晚几天升级通常只是少个功能；③ 开源工具本来也强制不了（用户可自行构建）。
- **替代机制（已实现）**：真正需要触达用户的事情用**本地状态**判断，不查版本号。例如"保险库还没上云、换机会锁死"这条，是 `bz unlock` 比对本地与云端实际状态后给出的（`backfillCloudVault`），零额外网络请求。
- **这条约束是承重的**：将来若有人提议加"检查更新"开关，请先重读上面第 ①②，并注意它会使"运行期只连百度"这个可被用户 tcpdump 验证的简单事实不再成立。
